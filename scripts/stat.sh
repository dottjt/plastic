...
accumulative_commit_word_stats() {
  ...
  do
    ...
    for chapter_file in $(ls ./chapters)
    do
      COMMIT_CHAPTER_WORD_COUNT=$(cat ./chapters/$chapter_file | sed 's/  <!--[^>]*-->//g' | sed 's/(##|####)//g' | wc -w | awk '{print $1}')
...
latest_total_branch_stats() {
  for chapter_file in $(ls ./chapters)
  do
    CHAPTER_WORD_COUNT=$(cat ./chapters/$chapter_file | sed 's/  <!--[^>]*-->//g' | sed 's/(##|####)//g' | wc -w | awk '{print $1}')
